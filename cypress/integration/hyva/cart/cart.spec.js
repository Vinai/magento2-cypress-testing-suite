...
describe("Cart tests", () => {
    /**
     * Parses a price string like "$92.00" and returns the numeric value.
     */
    ...
    it("Can change the quantity in the cart", () => {
        cy.get(cart.qtyInputField).clear();
        cy.get(cart.qtyInputField).type("2{enter}");
        cy.get(cart.qtyInputField).should("have.value", "2");
    });
    ...
    it("Cannot add an invalid coupon", () => {
        ...
        cy.get(cart.messageToast).should(
            "include.text",
            'The coupon code "wrong coupon code" is not valid.'
        );

        if (!isMobileHyva()) {
            cy.get(cart.messageToast).should("be.visible");
        }
    });
